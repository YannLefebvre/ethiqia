      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "24px 14px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <p style={{ fontFamily: "'Source Serif 4', serif", fontStyle: "italic", color: "#a09888", fontSize: 12, marginTop: 4 }}>
            {isAdmin ? "Mode administrateur" : "Cliquez sur une carte pour voter"}
          </p>
        </div>
              ...
              {questions.map(q => {
                ...
                return (
                  <button key={q.id} className={`num-card${uv === "A" ? " voted-a" : uv === "B" ? " voted-b" : ""}`} onClick={() => setActiveCard(q.id)} title={q.titre}>
                    ...
                    {tot > 0 && <span style={{ fontSize: 8, color: "#a09888" }}>{tot}v</span>}
                  </button>
                );
              })}
            </div>
          ...
          return (
            <div key={q.id} className="list-card">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                ...
                {tot > 0 && <div style={{ height: 3, borderRadius: 2, background: "rgba(255,255,255,0.08)", overflow: "hidden" }}>
        ...
        {status !== "loading" && view === "summary" && (
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.1)", padding: "14px 18px" }}>
            {questions.map(q => {
              const c = counts[q.id] || emptyCount(); const tot = c.A + c.B;
              const pctA = tot > 0 ? Math.round((c.A / tot) * 100) : null;
              const uv = voted[q.id];
              return (
                <div key={q.id} className="summary-row">
                  <div style={{ width: 22, height: 22, borderRadius: "50%", background: uv === "A" ? "rgba(79,195,247,0.2)" : uv === "B" ? "rgba(206,147,216,0.2)" : "rgba(255,255,255,0.06)", color: uv === "A" ? "#4fc3f7" : uv === "B" ? "#ce93d8" : "#665e52", fontSize: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>{q.id}</div>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 1px", fontFamily: "'Playfair Display', serif", fontSize: 12.5, color: "#f5efe0" }}>{q.titre}</p>
                    <p style={{ margin: "0 0 4px", fontSize: 10.5, color: "#665e52", fontStyle: "italic" }}>{q.situation}</p>
                    {uv && <p style={{ margin: "0 0 4px", fontSize: 11, color: uv === "A" ? "#4fc3f7" : "#ce93d8" }}>✓ Votre choix : {uv === "A" ? q.altA : q.altB}</p>}
                    {tot > 0 ? (
                      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                        <span style={{ fontSize: 11, color: "#4fc3f7" }}>Opt.1 : <b>{c.A}</b></span>
                        <span style={{ fontSize: 11, color: "#ce93d8" }}>Opt.2 : <b>{c.B}</b></span>
                        <span style={{ fontSize: 10, color: "#554e46" }}>({tot}v — {pctA}%/{100 - pctA}%)</span>
                        <div style={{ flex: 1, minWidth: 60, height: 3, background: "rgba(255,255,255,0.08)", borderRadius: 2, overflow: "hidden" }}>
                    ...
                    ) : <span style={{ fontSize: 10, color: "#443d36", fontStyle: "italic" }}>Aucun vote</span>}
                  </div>
                </div>
              );
            })}
